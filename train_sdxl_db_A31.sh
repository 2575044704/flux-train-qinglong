...
gradient_accumulation_steps=3 # 梯度累加数量，变相放大batchsize的倍数
...
lr_scheduler_num_cycles=10       # cosine_with_restarts restart cycles | 余弦退火重启次数，仅在 lr_scheduler 为 cosine_with_restarts 时起效。
...
lr_scheduler_type="torch.optim.lr_scheduler.CosineAnnealingWarmRestarts"
...
learning_rate_te="1e-5"      #文本编码器学习率 SD1.5/SD2.1

#SDXL_db
diffuser_xformers=0      #开启diffuser的xformers
learning_rate_te1="1e-5" #文本编码器1学习率
learning_rate_te2="1e-5" #文本编码器2学习率
...
if [[ $optimizer_type == "Lion" ]]; then
  ...
  extArgs+=("--optimizer_type=$optimizer_type" "--optimizer_args weight_decay=0.1 betas=.9,.99")
...
if [ $lr_scheduler_type ]; then
  extArgs+=("--lr_scheduler_type=$lr_scheduler_type" "--lr_scheduler_args eta_min=1e-06 T_0=9099")
fi
